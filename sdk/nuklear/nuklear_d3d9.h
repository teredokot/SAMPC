...
nk_d3d9_create_state()
{
	...
	viewport.X = 0;
	viewport.Y = 0;
	viewport.Width = d3d9.width;
	viewport.Height = d3d9.height;
	viewport.MinZ = 0.0f;
	viewport.MaxZ = 1.0f;
	IDirect3DDevice9_SetViewport(d3d9.device, &viewport);
	...
	{
		const float L = 0.5f;
		const float R = (float)d3d9.width + 0.5f;
		const float T = 0.5f;
		const float B = (float)d3d9.height + 0.5f;
		...
		IDirect3DDevice9_SetTransform(d3d9.device, D3DTS_WORLD, &mat_identity);
		IDirect3DDevice9_SetTransform(d3d9.device, D3DTS_VIEW, &mat_identity);
		IDirect3DDevice9_SetTransform(d3d9.device, D3DTS_PROJECTION, &mat_projection);
	}
}
...
nk_d3d9_render(enum nk_anti_aliasing AA)
{
	...
	hr = IDirect3DDevice9_CreateStateBlock(d3d9.device, D3DSBT_ALL, &state);
	NK_ASSERT(SUCCEEDED(hr));

	// Backup the DX9 transform (without it, game's viewport will broke)
	D3DMATRIX last_world, last_view, last_projection;
	IDirect3DDevice9_GetTransform(d3d9.device, D3DTS_WORLD, &last_world);
	IDirect3DDevice9_GetTransform(d3d9.device, D3DTS_VIEW, &last_view);
	IDirect3DDevice9_GetTransform(d3d9.device, D3DTS_PROJECTION, &last_projection);
	...
	IDirect3DDevice9_SetTransform(d3d9.device, D3DTS_WORLD, &last_world);
	IDirect3DDevice9_SetTransform(d3d9.device, D3DTS_VIEW, &last_view);
	IDirect3DDevice9_SetTransform(d3d9.device, D3DTS_PROJECTION, &last_projection);

	IDirect3DStateBlock9_Apply(state);
